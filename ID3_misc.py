...
sample = pd.read_csv('tfidf.train.csv')
...
prediction_rows= []
test = pd.read_csv('tfidf.test.csv')
...
prediction_rows = []
# print("depth",cal_tree_depth(root,0))
print("eval accuracy",accuracyCal(sample,root))
prediction_rows= []
test = pd.read_csv('tfidf.eval.anon.csv')
t = accuracyCal(test,root)
print("eval accuracy",t)

### for eval ####
prediction_rows= []
eval = pd.read_csv('tfidf.eval.anon.csv')
p = accuracyCal_eval(eval,root)
for i in range(len(p)):
    if p[i] == -1:
        p[i] = 0
with open('ID3_submission_tfidf.csv', 'w', newline='') as file:
